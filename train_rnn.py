...
def build_rnn_classifier(timesteps : int, vector_size : int) -> Model:
    ...
    return Sequential([
        ...
        SimpleRNN(64, return_sequences = True), #32)
        SimpleRNN(64),
...
if (__name__ == "__main__"):
    ...
    rnn_classifiers = {
        "rnn": {
            "mfcc": [
                build_rnn_classifier(**rnn_parameters["rnn"]["mfcc"]) for _ in range(num_folds)
            ],
            "melc": [
                build_rnn_classifier(**rnn_parameters["rnn"]["melc"]) for _ in range(num_folds)
            ]
        }
    }
    for model_type in [ "rnn" ]:
        for feature_type in [ "mfcc", "melc" ]:
            for model in rnn_classifiers[model_type][feature_type]:
                model.compile(optimizer = Adam(learning_rate = 1e-3), loss = "categorical_crossentropy", metrics = [ "Accuracy" ])
    print(rnn_classifiers)
    rnn_classifiers["rnn"]["mfcc"][0].summary()
...
if (__name__ == "__main__"):
    history_name : str = "/content/drive/MyDrive/multimedia_hw4_train_history/rnn/{}.json".format(datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(os.path.dirname(history_name), exist_ok = True)
    rnn_train_parameters = {
        "epochs": 500,
        "patience": 10
    }
    rnn_train_history = {
    ...
    #     rnn_train_history["rnn"]["mfcc"].append(train_classifier(rnn_classifiers["rnn"]["mfcc"][val_idx], mfcc_loaders, validation_label = validation_label, **rnn_train_parameters))
    #     json.dump(rnn_train_history, open(history_name, mode = "w"))
    #     gc.collect()
    #     clear_session()
    for val_idx, validation_label in enumerate([ "1", "2", "3", "4", "5" ]):
        rnn_train_history["rnn"]["melc"].append(train_classifier(rnn_classifiers["rnn"]["melc"][val_idx], melc_loaders, validation_label = validation_label, **rnn_train_parameters))
        json.dump(rnn_train_history, open(history_name, mode = "w"))
        gc.collect()
        clear_session()
